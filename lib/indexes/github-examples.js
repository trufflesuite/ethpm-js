...
module.exports = {
  initialize: function(options, callback) {
    ...
    var  ethpm_spec_path  = require.resolve('ethpm-spec');
    if (ethpm_spec_path === null) {
      callback(new Error('ethpm-spec dependency not found (possibly "npm install" helps)' ));
    }
    //GLOBAL - change this to become module-scope (refactor, without changing code functionality/api etc.)
    ethpm_spec_dir = path.dirname(ethpm_spec_path);
    ...
    series([
      ...
      function(c) {
        ...
        parallel(lockfiles_requests, function(err, results) {
          ...
          examples.forEach(function(package_name) {
  ...
  {
    var fullpath = path.join(global.ethpm_spec_dir, "examples", examples_path);
    var content  = '';
  ...
  // This should be removed completely, or replaced by a general library call
  // if kept, protocol&host options should be placed external to the code
  get_remote: function(examples_path, callback) {
    var options = {
      protocol: "https",
